...
export type TimelineProps = SectionVariantProps & {
  heading: Omit<SectionHeadingProps, "className">;
  steps: TimelineStep[];
};

/**
 * Numbered process timeline — "how we work" for contractors, builders, and any
 * service business that sells a process.
 */
          ...
          {steps.map((step, index) => (
            <StaggerItem key={step.title} className="relative flex gap-5 pb-10 last:pb-0">
              {index < steps.length - 1 ? (
                <span aria-hidden="true" className="bg-border absolute top-10 left-5 h-full w-px" />
              ) : null}
